...
class MySQL_Status:
    ...
    def connect_server(self, shellbash, hostname, port, username, password):
    ...
    def check_mysql_process(self):
        """检测mysql进程"""
        res = self.connect_server(shellbash='netstat -anpt | grep mysqld | wc -l', hostname=self.hostname,
    ...
    def check_mysql_restart(self):
        """重启mysql进程"""
        self.connect_server(shellbash='/etc/init.d/mysqld restart', hostname=self.hostname, port=self.port,
    ...
    def check_mysql_status(self, status_code):
        if status_code == "0":
            ...
            if restart_code == "0":
                print("未能检测到MySQL服务，尝试重启失败，请及时查看！！！")
                self.dingding(webhook, secret, dingding_text="未能检测到MySQL服务，尝试重启失败，请及时查看！！！")
            else:
...
if __name__ == '__main__':
    result = MySQL_Status(hostname='', port='', username='', password='')
